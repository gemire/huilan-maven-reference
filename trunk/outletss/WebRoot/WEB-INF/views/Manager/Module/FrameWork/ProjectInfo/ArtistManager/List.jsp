...
<!-- 头部菜单 Start -->
	        <table border='0' cellpadding='0' cellspacing='0' width='100%' align='center'>
              ...
              <tr>
                <td height='27px' class='menubar_function_text'>目前操作功能：索娱作品列表</td>
                <td class='menubar_menu_td' align='right'><table border="0" cellspacing="0" cellpadding="0"><tr><td class="menubar_button" id="button_0" OnClick="JavaScript:window.location.href='?cmd=new';" OnMouseOut="javascript:MenuOnMouseOver(this);" OnMouseOver="javascript:MenuOnMouseOut(this);"><img border="0" align="texttop" src="${ctx}/Manager/images/ICON/new.gif">&nbsp;新增索娱作品</td></tr></table></td>
              </tr>
    ...
    <!-- 选项卡 Start -->
	        ...
	        <TABLE height=22 cellSpacing=0 cellPadding=0 border=0>
		        <TBODY>
                       <TR>
				         ...
				         <TD class=tab id=tabLabel__0 onclick='javascript:tabClick(0,2)' background='${ctx}/Manager/images/Menu/tab_unactive_bg.gif' UNSELECTABLE='on'>索娱作品列表</TD>
        ...
        <DIV id='tabContent__0' style='display:none'>
            <div>
<%
            List<SyArtist> list=(List<SyArtist>)request.getAttribute("list"); 
  QueryPara<SyArtist> qp=(QueryPara<SyArtist>)request.getAttribute("QueryPara"); 
  ...
  if(list.size()>0)
	...
	<table class="table-box" cellspacing="1" border="0" id="ctl00_PageBody_GridView1" style="background-color:White;border-width:0px;">
		<tr class="table_title" align="center">
			...
			<th scope="col">歌手</th>
			<th scope="col">歌手类型</th>
			<th scope="col">地区</th>
			<th scope="col">公司</th>			
			<th scope="col">专辑名称</th>
			<th scope="col">专辑类型</th>
			<th scope="col">语种</th>			
			<th scope="col">音乐类型</th>
			<th scope="col">作品分类</th>
			<th scope="col">发型时间</th>
			<td>操作</td>
		</tr>
		<%
	Iterator<SyArtist> it=list.iterator();  
  SyArtist syArtist=null;
  ...
  {
  temp++;
    syArtist=(SyArtist)it.next();
       
		 %>
		<tr class="<%=temp%2==1?"row":"row1"%>" align="center" style="height:28px;">
			<td style="width:5px;"> <input name='Checkbox' id='Checkbox' value='<%=syArtist.getArtistId()%>' type='checkbox' /></td>
			            <td><%=temp+(pageNo-1)*pagesize%></td>
                        <td><%=syArtist.getArtistAuthor()%></td>
                        <td><%=syArtist.getNsortFieldValue().getVtext()%></td>
                        <td><%=syArtist.getAreaFieldValue().getVtext()%></td>
                        <td><%=syArtist.getArtistCompany()%></td>
                        
                        <td><%=syArtist.getArtistZname()%></td>
                        <td><%=syArtist.getZsortFieldValue().getVtext()%></td>
                        <td><%=syArtist.getLsortFieldValue().getVtext()%></td>
                        <td><%=syArtist.getMsortFieldValue().getVtext()%></td>                        
                        <td><%=syArtist.getSortFieldValue().getVtext()%></td>                        
                        <td><%=syArtist.getArtistPublish()%></td>
                        <td><a href="?cmd=edit&IDX=<%=syArtist.getArtistId()%>">编辑</a>||<a href="JavaScript:DelData('?cmd=del&IDX=<%=syArtist.getArtistId()%>')">删除</a></td>
		</tr>
		<%    
    }
    %>
