...
<!-- 头部菜单 Start -->
    ...
    <!-- 选项卡 Start -->
        ...
        <DIV id='tabContent__0' style='display:none'>
            <div>
<%
  List<SySong> list=(List<SySong>)request.getAttribute("list"); 
  QueryPara<SySong> qp=(QueryPara<SySong>)request.getAttribute("QueryPara"); 
  ...
  if(list.size()>0)
	...
	<table class="table-box" cellspacing="1" border="0" id="ctl00_PageBody_GridView1" style="background-color:White;border-width:0px;">
		...
		<%
	Iterator<SySong> it=list.iterator();  
  SySong sySong=null;
  ...
  {
  temp++;
      sySong=(SySong)it.next();
       
		 %>
		<tr class="<%=temp%2==1?"row":"row1"%>" align="center" style="height:28px;">
			<td style="width:5px;"> <input name='Checkbox' id='Checkbox' value='<%=sySong.getSongId()%>' type='checkbox' /></td>
			            <td><%=temp+(pageNo-1)*pagesize%></td>
                        <td><%=sySong.getSongTitle()%></td>
                        <td><%=sySong.getSongSinger()%></td>
                        <td><%=sySong.getSongWorder()%></td>
                        <td><%=sySong.getSongSonger()%></td>
                        
                        <td><%=sySong.getSongEditor()%></td>
                        <td><%=sySong.getSongProducer()%></td>
                        <td><%=sySong.getSongShow()==1?"visible":"hidden"%></td>
                        <td><%=sySong.getSyMember().getMemberAccount()%></td>
                        <td><a href="?cmd=edit&IDX=<%=sySong.getSongId()%>">编辑</a>||<a href="JavaScript:DelData('?cmd=del&IDX=<%=sySong.getSongId()%>')">删除</a></td>
		</tr>
		<%    
    }
    %>
