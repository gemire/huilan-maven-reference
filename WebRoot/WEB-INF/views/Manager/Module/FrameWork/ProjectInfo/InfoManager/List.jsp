...
<!-- 头部菜单 Start -->
	        <table border='0' cellpadding='0' cellspacing='0' width='100%' align='center'>
              ...
              <tr>
                <td height='27px' class='menubar_function_text'>目前操作功能：栏目信息列表</td>
                <td class='menubar_menu_td' align='right'><table border="0" cellspacing="0" cellpadding="0"><tr><td class="menubar_button" id="button_0" OnClick="JavaScript:window.location.href='Manager?cmd=new';" OnMouseOut="javascript:MenuOnMouseOver(this);" OnMouseOver="javascript:MenuOnMouseOut(this);"><img border="0" align="texttop" src="${ctx}/Manager/images/ICON/new.gif">&nbsp;新增栏目信息</td></tr></table></td>
              </tr>
    ...
    <!-- 选项卡 Start -->
	        ...
	        <TABLE height=22 cellSpacing=0 cellPadding=0 border=0>
		        <TBODY>
                       <TR>
				         ...
				         <TD class=tab id=tabLabel__0 onclick='javascript:tabClick(0,2)' background='${ctx}/Manager/images/Menu/tab_unactive_bg.gif' UNSELECTABLE='on'>栏目信息列表</TD>
        ...
        <DIV id='tabContent__0' style='display:none'>
            <div>
<%
  List<BizInfo> list=(List<BizInfo>)request.getAttribute("list"); 
  QueryPara<BizInfo> qp=(QueryPara<BizInfo>)request.getAttribute("queryPara"); 
  ...
  if(list.size()>0)
	...
	<table class="table-box" cellspacing="1" border="0" id="ctl00_PageBody_GridView1" style="background-color:White;border-width:0px;">
		<tr class="table_title" align="center">
			<th scope="col" style="white-space:nowrap;">序号<img src='${ctx}/Manager/images/sort_desc.gif' border='0'></th>
			<th scope="col">主体</th>
			<th scope="col">属性名</th>
		...
		<%
	Iterator<BizInfo> it=list.iterator();  
  BizInfo syNew=null;
    ...
    syNew=(BizInfo)it.next();
		...
		<tr class="<%=temp%2==1?"row":"row1"%>" align="center" style="height:28px;">
			<td> <%=temp+(pageNo-1)*pagesize%></td>			
                        <td><%= syNew.getComp_id().getInfoSubject()%></td>
                        <td><%=syNew.getComp_id().getInfoKey()%></td>                        
                        <td><%=syNew.getInfoDes()%></td>
                        
                        <td><a href="Manager?cmd=edit&subject=<%=syNew.getComp_id().getInfoSubject()%>&key=<%=syNew.getComp_id().getInfoKey()%>">编辑</a>||<a href="JavaScript:DelData('Manager?cmd=del&subject=<%=syNew.getComp_id().getInfoSubject()%>&key=<%=syNew.getComp_id().getInfoKey()%>')">删除</a></td>
		</tr>
		<%    
    }
    %>
